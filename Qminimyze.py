...
def get_minterms(df, output_var):
    ...
    for _, row in df.iterrows():
        if row[output_var] == 1:
            minterms.append(tuple(row[:-4]))  # Берем только входные переменные
    return minterms
...
file_path = "/home/kevgen/Documents/таблица после правок вострова.xlsx"
xls = pd.ExcelFile(file_path)
df = xls.parse(sheet_name='Sheet1')

# Переименовываем столбцы для удобства
df.columns = ['a', 'b', 'c', 'q1', 'q2', 'q3', 'q4', '_', 'Q1', 'Q2', 'Q3', 'Q4']
...
minimized_expressions = {
    f'Q{i + 1}': minimize_boolean_function(get_minterms(df, f'Q{i + 1}'), variables) for i in range(4)
}
